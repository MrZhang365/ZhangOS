...
async function getAppList() {
    const result = JSON.parse(await zhr('system/app-list', 'GET'))
    return result
}
...
async function showAppList() {
    ...
    list.forEach(a => {
        ...
        link.classList.add('mdui-list-item', 'mdui-ripple')
        link.textContent = a.name
        link.href = a.homepage
        link.setAttribute('mdui-tooltip', JSON.stringify({
            content: a.explain,
        }))
        $('app-list').appendChild(link)
    })
}
